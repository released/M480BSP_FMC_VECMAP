...
#define FLAG_PROJ_BOOT_PROCESS                 			(flag_PROJ_CTL.bit1)
...
volatile uint32_t addr_vector_map = 0;
...
{
    if (addr_vector_map == 0x00)
    {
        printf("under BANK1 : 0x00000 , reboot to VECT : 0x40000\r\n");
        FMC_SetVectorPageAddr(0x40000);
        SYS_ResetCPU();

    }
    else if (addr_vector_map == 0x40000)
    {
        printf("under BANK2 : 0x40000 , reboot to VECT : 0x00000\r\n");
...
void GET_VECMAP(void)
{
    SYS_UnlockReg();
    FMC_Open();

    addr_vector_map = FMC_GetVECMAP();
...
void loop(void)
{
    ...
    if (FLAG_PROJ_TIMER_PERIOD_1000MS)//(is_flag_set(flag_timer_period_1000ms))
    {
        ...
        printf("%s(timer) : %4d , vector map : 0x%8X\r\n",__FUNCTION__,LOG1++ , addr_vector_map);
        PH0 ^= 1;             
    }

    if (FLAG_PROJ_BOOT_PROCESS)
    {
        FLAG_PROJ_BOOT_PROCESS = 0;
        BOOT_PROCESS();        
    }
}
